        ...
        for (int i = 0; i < wordsChar.length-1; i++){
            ...
            for (int j = i+1; j < wordsChar.length; j++){
                for (int k = 0; k < wordsChar[j].length; k++){
                    node:
                    if (wordsChar[j][k] == wordsChar[i][0]
                            && wordsChar[i].length <= wordsChar[j].length - k){
                        int temp = k + 1;
                        for (int g = 1; g < wordsChar[i].length; g++){
                            if (temp < wordsChar[j].length &&
                                    wordsChar[i][g] != wordsChar[j][temp++]){
                                break node;
                            }
                        }
                        if (k+wordsChar[i].length != wordsChar[j].length)
                            continue;
                        delList.add(i);
                        break node2;
                    }
